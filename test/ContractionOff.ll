...
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv.default
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.spv.default -to-text -o - | FileCheck %s --check-prefixes CHECK,CHECK-ON
; RUN: spirv-val %t.spv.default

; RUN: llvm-as %s -o %t.bc
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv.off --spirv-fp-contract=off
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.spv.off -to-text -o - | FileCheck %s --check-prefixes CHECK,CHECK-OFF
; RUN: spirv-val %t.spv.off

; RUN: llvm-as %s -o %t.bc
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv.fast --spirv-fp-contract=fast
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.spv.fast -to-text -o - | FileCheck %s --check-prefixes CHECK,CHECK-FAST
; RUN: spirv-val %t.spv.fast

; RUN: llvm-as %s -o %t.bc
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv.on --spirv-fp-contract=on
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.spv.on -to-text -o - | FileCheck %s --check-prefixes CHECK,CHECK-ON
