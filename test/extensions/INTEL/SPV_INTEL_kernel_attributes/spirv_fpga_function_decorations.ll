...
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers --spirv-ext=+SPV_INTEL_fpga_invocation_pipelining_attributes %t.bc -o %t.spv
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers --spirv-ext=+SPV_INTEL_fpga_invocation_pipelining_attributes -r %t.spv --spirv-target-env=SPV-IR -o %t.rev.bc
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -r --spirv-ext=+SPV_INTEL_fpga_invocation_pipelining_attributes %t.spv --spirv-target-env=SPV-IR -o %t.rev.bc
