; RUN: llvm-as < %s | llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -spirv-ext=+SPV_INTEL_function_pointers -o %t.spv
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.spv -spirv-ext=+SPV_INTEL_function_pointers -to-text -o - | FileCheck %s --check-prefix=CHECK-SPIRV
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -r %t.spv -o - | llvm-dis | FileCheck %s --check-prefix=CHECK-LLVM
