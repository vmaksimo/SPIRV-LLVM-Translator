...
; RUN: not llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc --spirv-ext=+SPV_INTEL_task_sequence 2>&1 \
; RUN: | FileCheck %s --check-prefix=CHECK-ERROR

; CHECK-ERROR: InvalidInstruction: Can't translate llvm instruction:
; CHECK-ERROR-NEXT: TaskSequenceCreateINTEL
; CHECK-ERROR-NEXT: Pipeline must be a 32 bit integer with the value bigger or equal to -1
...
entry:
  ...
  %call.i1 = call spir_func noundef target("spirv.TaskSequenceINTEL") @_Z31__spirv_TaskSequenceCreateINTEL(ptr noundef nonnull @_Z4multii, i32 noundef -2, i32 noundef -1, i32 noundef 17, i16 noundef zeroext 1) #3
