; fp8 matrix sources require the sub-extension SPV_INTEL_subgroup_matrix_multiply_accumulate_float8
; in addition to the parent and the new extension itself.
...
; Without the float8 sub-extension, translation must fail.
...
; CHECK-LLVM: %{{.*}} = call spir_func <8 x float> @_Z51__spirv_SubgroupScaledMatrixMultiplyAccumulateINTELiDv8_sDv8_iDv8_fcci(i32 32, <8 x i16> %{{.*}}, <8 x i32> %{{.*}}, <8 x float> %{{.*}}, i8 %{{.*}}, i8 %{{.*}}, i32 3162624)
...
entry:
  ...
  ;         = 0x30C000 = 3162624
  %call = call spir_func <8 x float> @_Z51__spirv_SubgroupScaledMatrixMultiplyAccumulateINTELiDv8_sDv8_iDv8_fcci(i32 32, <8 x i16> %sM8, <8 x i32> %iM8, <8 x float> %fM8, i8 %sA, i8 %sB, i32 3162624)
  ret void
}
