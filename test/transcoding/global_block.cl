...
// RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers --spirv-max-version=1.1 %t.bc -spirv-text -o - | FileCheck %s --check-prefix=CHECK-SPIRV
...
// RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -r %t.spirv1.1.spv -o - | llvm-dis | FileCheck %s --check-prefix=CHECK-LLVM

// RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers --spirv-max-version=1.4 %t.bc -spirv-text -o - | FileCheck %s --check-prefixes=CHECK-SPIRV1_4,CHECK-SPIRV
...
// RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -r %t.spirv1.4.spv -o - | llvm-dis | FileCheck %s --check-prefix=CHECK-LLVM
