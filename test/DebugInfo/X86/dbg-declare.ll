...
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv --spirv-ext=+SPV_INTEL_variable_length_array
...
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv --spirv-ext=+SPV_INTEL_variable_length_array --spirv-debug-info-version=nonsemantic-shader-100
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -r %t.spv -o - | llvm-dis -o %t.ll
; RUN: llc < %t.ll -O0 -mtriple x86_64-apple-darwin | FileCheck %s
; RUN: llc < %t.ll -O0 -mtriple x86_64-apple-darwin -filetype=obj \
; RUN:     | llvm-dwarfdump -v - --debug-info | FileCheck %s --check-prefix=DWARF

; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers %t.bc -o %t.spv --spirv-ext=+SPV_INTEL_variable_length_array --spirv-debug-info-version=nonsemantic-shader-200
; RUN: llvm-spirv --spirv-ext=+SPV_KHR_untyped_pointers -r %t.spv -o - | llvm-dis -o %t.ll
; RUN: llc < %t.ll -O0 -mtriple x86_64-apple-darwin | FileCheck %s
; RUN: llc < %t.ll -O0 -mtriple x86_64-apple-darwin -filetype=obj \
; RUN:     | llvm-dwarfdump -v - --debug-info | FileCheck %s --check-prefix=DWARF
